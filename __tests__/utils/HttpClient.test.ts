...
describe("HttpClient", () => {
  ...
  describe("baseRequest", () => {
    test("It's call with params fetch is call with these params", () => {
      // Arrange
    ...
    test("It's call with absolute url and ignore with base url", async () => {
      // Arrange
      const path = "https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch";
    ...
    test("Response interceptor change response", async () => {
      ...
      httpclient.responseInterceptor = jest.fn().mockReturnValue(resIntercepted);
  ...
  describe("objectToQueryParams", () => {
    test("should return flatten query paramaters as get data pattern", () => {
      ...
      const expected = `name=${query.name}&age=${query.age}&isOk=${query.isOk}&json=${JSON.stringify(
        query.json
      )}&array=${JSON.stringify(query.array)}`;
  ...
  describe("get method", () => {
    ...
    test("Should call baseRequest with returned url from objectToQueryParams", () => {
      ...
      httpclient.objectToQueryParams = jest.fn().mockReturnValue(expectedUrlParams);
