...
export interface IHttpClient {
  ...
  responseInterceptor?: <T>(response: Promise<TypedResponse<T>>) => Promise<TypedResponse<T>>;

  baseRequest<TBody extends BodyInit, TResponse>(params: BaseRequestParam<TBody>): Promise<TypedResponse<TResponse>>;
  get<TQuery extends object, TResponse>(params: RequestWithParam<TQuery>): Promise<TypedResponse<TResponse>>;
  post<TBody extends BodyInit, TResponse>(params: RequestWithBody<TBody>): Promise<TypedResponse<TResponse>>;
  path<TBody extends BodyInit, TResponse>(params: RequestWithBody<TBody>): Promise<TypedResponse<TResponse>>;
  ...
  delete<TQuery extends object, TResponse>(params: RequestWithParam<TQuery>): Promise<TypedResponse<TResponse>>;
...
export class HttpClient implements IHttpClient {
  ...
  constructor(baseUrl?: string | undefined, globalHeaders?: Header | undefined) {
  ...
  baseRequest<TBody extends BodyInit, TResponse>(params: BaseRequestParam<TBody>): Promise<TypedResponse<TResponse>> {
    ...
    if (!this.baseUrl && !url) throw new Error("Unable to perform a query on an undefined url");
